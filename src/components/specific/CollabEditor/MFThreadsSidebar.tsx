// slightly modified version of: https://github.com/TypeCellOS/BlockNote/blob/0ad2da6bde27d8fac3665074470b0f29e3a1a95e/packages/react/src/components/Comments/ThreadsSidebar.tsx#L165
...
import {Thread, useBlockNoteEditor, useThreads} from '@blocknote/react'

type ThreadItemProps = {
  thread: ThreadData
  selectedThreadId?: string
  // @ts-expect-error: ignore for now
  editor: BlockNoteEditor<unknown, unknown, unknown>
  maxCommentsBeforeCollapse?: number
  referenceText: string
}

// copied from https://github.com/TypeCellOS/BlockNote/blob/0ad2da6bde27d8fac3665074470b0f29e3a1a95e/packages/react/src/hooks/useUIPluginState.ts as not exported
    ...
    const onBlur = useCallback(
      (event: React.FocusEvent) => {
        ...
        const parentThreadElement =
          event.relatedTarget instanceof Node ? event.relatedTarget.closest('.bn-thread') : null
...
export function ThreadsSidebar(props: {
  ...
  const state = useUIPluginState(editor.comments.onUpdate.bind(editor.comments))
      ...
      if (!thread.resolved) {
        if (props.filter === 'open' || props.filter === 'all') {
          ret.push({
            thread,
            // @ts-expect-error: ignore for now
            referenceText: getReferenceText(editor, state?.threadPositions.get(thread.id)),
          })
        }
      } else {
